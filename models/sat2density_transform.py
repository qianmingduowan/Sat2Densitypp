...
def get_original_coord(opt):
    ...
    elif opt.data.dataset in ['CVUSA']:   ### no 
...
def render(opt,feature,voxel,pano_direction,PE=None):
    ...
    if opt.data.origin_H_W is None: ### origin_H_W is the photo taken space in regular coordinate
        ...
        origin_H,origin_w = torch.ones([BS,1])*opt.data.origin_H_W[0],torch.ones([BS,1])*opt.data.origin_H_W[1]
    origin = torch.cat([origin_w,origin_z,origin_H],dim=1).cuda()[:,None,None,:]  ## w,z,h, samiliar to NERF coordinate definition
    sample_len = ((torch.arange(opt.data.sample_number)+1)*(sample_total_length/opt.data.sample_number)).cuda()
    ...
    #### tbd
    voxel = torch.cat([torch.ones(voxel.size(0),1,voxel.size(2),voxel.size(3)).cuda()*1000,voxel],1)
...
if __name__ == '__main__':
    ...
    opt.data.origin_H_W =None
    BS = 1
    opt.data.sample_total_length = 1
    sat_name = './data/CVACT/satview_correct/__-DFIFxvZBCn1873qkqXA_satView_polish.png'
    ...
    with torch.autograd.profiler.profile(enabled=True, use_cuda=True, record_shapes=False, profile_memory=False) as prof:
        output =render(opt,a,voxel,pano_direction)
    rgb = output['rgb']
    ...
    torchvision.utils.save_image(torch.cat([rgb,pano],2),  'aaa.png')
