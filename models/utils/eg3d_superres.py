...
class Superresolution_s2d_Hybrid8XDC(torch.nn.Module):
    """Defines the super-resolution network used for generating 512x512 images
    in EG3D."""
    ...
    def forward(self, rgb, x, ws, **block_kwargs):
        ws = ws[:, -1:, :].repeat(1, 3, 1)
        if x.shape[-1] //2 != self.input_resolution:
            x = torch.nn.functional.interpolate(x,
                                                size=(self.input_resolution,
                                                      self.input_resolution),
                                                mode='bilinear',
...
class Superresolution_s2d_Hybrid4X(torch.nn.Module):
    """Defines the super-resolution network used for generating 256x256 images
    in EG3D."""
...
class Superresolution_s2d_sat(torch.nn.Module):
    """Defines the super-resolution network used for generating 256x256 images
    in EG3D."""
...
class Superresolution_s2d_Hybrid2X(torch.nn.Module):
    """Defines the super-resolution network used for generating 128x128 images
    in EG3D."""
...
class SynthesisBlockNoUp(torch.nn.Module):
    ...
    def forward(self,
        ...
        if self.in_channels == 0:
            ...
            assert x.shape[2]*x.shape[3] == self.resolution**2
            # misc.assert_shape(
