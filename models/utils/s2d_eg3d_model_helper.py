...
from utils.camera_utils import camera_9d_to_16d
...
class SynthesisLayer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if use_noise:
            ...
            #         torch.randn([resolution//2, resolution*2]))
            if self.for_pano:
                self.register_buffer('noise_const',
                                    torch.randn([resolution//2, resolution*2]))
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        assert x.shape[2] * x.shape[3] == in_resolution**2
        # misc.assert_shape(
        ...
        if self.use_noise and noise_mode == 'random':
            if self.for_pano:
                noise = torch.randn(
                    [x.shape[0], 1, self.resolution//2, self.resolution*2],
...
class SynthesisBlock(torch.nn.Module):
        ...
        if in_channels == 0:
            if for_pano:
                self.const = torch.nn.Parameter(
                    torch.randn([out_channels, resolution//2, resolution*2]))
            else:
                self.const = torch.nn.Parameter(
    ...
    def forward(self,
        ...
        if self.in_channels == 0:
            ...
            assert self.resolution ** 2 //4 == x.shape[2] * x.shape[3] 
            assert x.shape[1] == self.in_channels
            # misc.assert_shape(x, [
        ...
        # ToRGB.
        if img is not None:
            assert img.shape[1] == self.img_channels
            assert img.shape[2] * img.shape[3] == self.resolution**2 //4
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        if max_block is not None:
            ...
            self.block_resolutions = self.block_resolutions[-(max_block):]
        channels_dict = {
        ...
        for res in self.block_resolutions:
            if res // 2 in channels_dict.keys():
                ...
                in_channels = channels_dict[res] if res !=4 else 0
...
class Generator_s2d(torch.nn.Module):
    """Defines the generator network.
    ...
    def __init__(
        ...
        **synthesis_kwargs,
        ...
        self.synthesis = SynthesisNetwork(w_dim=w_dim,
                                          ...
                                          **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = Sat_MappingNetwork_disentangle(z_dim=z_dim,
                                      c_dim=c_dim,
                                      w_dim=w_dim,
                                      in_resolution=256,
                                      in_channels=3,
...
class Encoder(torch.nn.Module):
    def __init__(self,
        ...
        **unused
        ...
        temp_block_resolutions = self.block_resolutions + [4] if max_block is  None else self.block_resolutions
        channels_dict = {res: min(channel_base // res, channel_max) for res in temp_block_resolutions}
        ...
        for res in self.block_resolutions:
            in_channels  = channels_dict[res] if res < img_resolution else 0
            tmp_channels = channels_dict[res]
            out_channels = channels_dict[res // 2] if res // 2  in channels_dict.keys() else tmp_channels
            use_fp16 = (res >= fp16_resolution)
    ...
    def forward(self, inputs, **block_kwargs):
        ...
        if self.output_mode in ['W', 'W+',]:
            ...
            if self.output_mode == 'W+':
                ...
                out = None
            outputs['ws'] = out
        elif  self.output_mode is None:
            outputs['feature'] = x

        return outputs


class Sat_MappingNetwork_disentangle(torch.nn.Module):
    def __init__(self,
        ...
        **unused,
        ...
        self.mapping_sat = mapping_sat
        self.geometry_layer = 7 if self.mapping_sat else 0
    ...
    def forward(self, z=None, c=None, sat_img=None, truncation_psi=1, truncation_cutoff=None, update_emas=False, **unused_kwargs):
        # Embed, normalize, and concat inputs.
        x = None
        with torch.autograd.profiler.record_function('input'):
            if self.z_dim > 0:
        ...
        if self.mapping_sat:
            ...
            misc.assert_shape(sat_img, [z.shape[0], self.in_channels, self.in_resolution, self.in_resolution])
            y = self.embed_mask(sat_img.to(torch.float32))['ws'] # B x 7 x w_dim
